...
class MainWindow(QWidget):
    ...
    def agregar_robot_en_posicion(self, row, col):
        ...
        z = 0.15  # Ligeramente sobre el suelo para visualizarlo mejor
    ...
    def reset_grid(self):
        """
        Restablece la cuadrícula y elimina todos los objetos de CoppeliaSim,
        con manejo de errores mejorado
        """
        try:
            print("Iniciando restablecimiento de la cuadrícula...")
            
            # Resetear la cuadrícula en la interfaz primero
            self.grid_manager = GridManager()
            ...
            if self.is_connected:
                ...
                # Paso 1: Detener la simulación si está en ejecución
                try:
                    self.sim_controller.stop_simulation()
                    import time
                    time.sleep(0.5)  # Esperar a que se detenga completamente
                except Exception as e:
                    print(f"Advertencia al detener simulación: {e}")
                
                # Paso 2: Eliminar cubos directamente por sus handles
                print("Eliminando cubos...")
                
                # Copiar las claves para evitar modificar el diccionario durante la iteración
                ...
                for pos in positions:
                    try:
                        ...
                        try:
                            # Intentar obtener posición para verificar si existe
                            self.sim_controller.sim.getObjectPosition(handle, -1)
                            
                            # Si llegamos aquí, el objeto existe, intentar eliminarlo
                        ...
                        # Eliminar el handle del registro de cualquier manera
                        del self.cube_handles[pos]
                        
                    except Exception as e:
                        print(f"Error al eliminar cubo en posición {pos}: {e}")
                
                # Paso 3: Intentar eliminar el robot
                if self.robot_handle is not None:
                    try:
                        try:
                            ...
                            # Obtener todos los hijos del robot
                            try:
                                children = self.sim_controller.sim.getObjectsInTree(self.robot_handle, 0, 0)
                                print(f"Encontrados {len(children)} componentes del robot")
                                
                                # Eliminar hijos individualmente, excepto el robot principal
                                for child in children:
                                    if child != self.robot_handle:
                                        try:
                                            self.sim_controller.sim.removeObject(child)
                                        except:
                                            pass
                            except:
                                pass
                            
                            # Ahora eliminar el robot principal
                ...
                # Paso 4: Limpieza final de objetos
                try:
                    # Buscar todos los objetos por nombre
                    all_objects = []
                    try:
                        # Intentar obtener todos los objetos en la escena
                        # Nota: Evitar getObjectsWithType que no existe en tu versión
                        all_objects = self.sim_controller.sim.getObjects()
                    except:
                        pass
                    
                    # Eliminar objetos con nombres relacionados con robots/cubos
                    for obj in all_objects:
                        try:
                            name = self.sim_controller.sim.getObjectName(obj).lower()
                            if any(keyword in name for keyword in ["pioneer", "p3dx", "sensor", "cube", "cubo", "obstacle"]):
                                try:
                                    self.sim_controller.sim.removeObject(obj)
                                    print(f"✅ Objeto adicional eliminado: {name}")
                                except:
                                    pass
                        except:
                            pass
